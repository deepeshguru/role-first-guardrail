...
def main():
    ...
    for r in rows:
        role = r.get("role", "?")
        role_counts[role] += 1
        if r.get("allowed"): role_allow[role] += 1
    ...
    out = {
        ...
        "by_role_allow_rate": {k: round(role_allow[k] / v, 4) for k, v in role_counts.items()},
        "by_intent_count": dict(sorted(intent_counts.items(), key=lambda kv: (-kv[1], kv[0]))),
